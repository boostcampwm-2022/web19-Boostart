...
router.get('/:user_id', authenticateToken, async (req: AuthorizedRequest, res) => {
  ...
  try {
    ...
    if (isNotFriend) return res.status(403).json({ msg: '친구가 아닌 사용자의 목표를 조회할 수 없어요.' });
...
router.post('/', authenticateToken, async (req: AuthorizedRequest, res) => {
  ...
  try {
    ...
    res.status(201).json({ msg: '목표 생성이 완료되었어요.' });
  } catch (error) {
    res.status(500).json({ msg: '서버 에러가 발생했어요.' });
  }
});
...
router.put('/:goal_idx', authenticateToken, async (req: AuthorizedRequest, res) => {
  ...
  try {
    ...
    res.status(200).json({ msg: '목표 수정이 완료되었어요.' });
  } catch {
    res.status(500).json({ msg: '서버 에러가 발생했어요.' });
  }
});
...
router.delete('/:goal_idx', authenticateToken, async (req: AuthorizedRequest, res) => {
  ...
  try {
    ...
    res.status(200).json({ msg: '목표가 삭제되었어요.' });
  } catch {
    res.status(500).json({ msg: '서버 에러가 발생했어요.' });
  }
});
