...
const Log = () => {
  ...
  dummyTaskList.forEach((task) => {
    tagMap.set(task.idx, task);
  });
  ...
  const handleTagWrapClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (activeTaskIdx === undefined || parseInt(activeTaskIdx) === activeTag) {
      ...
      calculateTime(tagMap.get(activeIdx).startedAt, tagMap.get(activeIdx).endedAt);
    }
  };
    ...
    <>
      {selectedElement !== null && (
        <S.SelectedItem x={mousePos[0]} y={mousePos[1]}>
          <span>{tagMap.get(selectedElement).startedAt}</span> {tagMap.get(selectedElement).title}
        </S.SelectedItem>
      )}
